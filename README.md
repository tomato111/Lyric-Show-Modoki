...
* タイムタグが正常に読み込めない場合がある不具合を修正  
* Panel.Path の2つ目以降のパスであいまい検索が行われない不具合を修正  

--v1.8.4(2018/03/27)-------------------------------------------------

* Style.AlignNoLyric が正しく反映されない場合がある不具合を修正  

--v1.8.3(2018/02/21)-------------------------------------------------

* 1つの検索プラグインのエラー表示を再変更  
...
* 9つの検索プラグインを修正  
　サイトの仕様変更に対応と、取得率の向上など  
* 1つの検索プラグインを追加  

(dplugin_Kashinavi.js は更新しないことにしたので各自で削除を。)  

(JScript Panel 1.3.1 で IStyleTextRender が削除されたため  
JScript Panel のサポートは 1.3.0 までになります。  
1.3.1 以降では動作しませんのでご注意ください。)  
...
* 4つの検索プラグインを修正  
　サイトの仕様変更に対応と、サーバーの応答が遅い時に固まらないようにした  

--v1.7.1(2017/06/07)-------------------------------------------------

* 設定 Plugin.Search.AutoSaveLrcTo を追加  
* 設定 Plugin.ViewLyrics.HighPriority を追加  
* 自動検索での切り替えで場合によってクラッシュする不具合を修正  
* BOM付きファイルは必ず該当する文字コードで読み込むようにした  

--v1.7.0(2017/06/03)-------------------------------------------------

* 1つの検索プラグインを追加  
　自動検索に追加した場合、Enterキーの隣にある"]"キーで結果を表示します  
* 編集画面で行数が少ないと正常に表示されない不具合を修正  
